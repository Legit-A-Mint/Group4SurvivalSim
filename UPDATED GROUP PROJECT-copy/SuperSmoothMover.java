...
public abstract class SuperSmoothMover extends Actor
{
    ...
    public <T extends Actor> T findClosestTarget(Class<T> targetClass, int intialSearchRange, int increaseSearchRange, int maxSearchRange) {
        double closestTargetDistance = 0;
        double distanceToActor;
        
        ArrayList<T> targets = (ArrayList<T>)getObjectsInRange(intialSearchRange, targetClass);
        T target;
        
        int range = intialSearchRange;
        
        
        while(targets.size() == 0 && range <= maxSearchRange){
            if (targets.size() == 0){ 
                targets = (ArrayList<T>)getObjectsInRange(range, targetClass);
                range += 200;
            } 
        ...
        if (targets.size() > 1)
        {
            ...
            closestTargetDistance = SimulationWorld.getDistance (this, target);
    ...
    public <T extends Actor> double distanceToClosestTarget(Class<T> targetClass, int intialSearchRange, int increaseSearchRange, int maxSearchRange) {
        double closestTargetDistance = 0;
        double distanceToActor;
